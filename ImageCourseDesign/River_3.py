...
# img = cv.blur(img, (9, 9))  # cv.blur 方法（均值滤波）比原图稍好
# img = cv.equalizeHist(img)  # 均衡化
# img = cv.blur(img, (3, 3))  # cv.blur 方法（均值滤波）
# # 灰度直方图
# histCV = cv.calcHist([img], [0], None, [256], [0, 256])  # 灰度直方图
# OTSU 全局阈值处理t5
ret, imgOtsu = cv.threshold(img, 0, 255, cv.THRESH_OTSU)  # 阈值分割, thresh=T
# 自适应局部阈值处理
...
_, imgTop = cv.threshold(
    img, 250, 255, cv.THRESH_BINARY)  # 高百分位阈值产生种子区域
