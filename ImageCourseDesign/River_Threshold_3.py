...
imgErode3 = cv.erode(th1, kernel=kernel)  # 图像腐蚀
kSize = (5, 5)  # 卷积核的尺寸
kernel = np.ones(kSize, dtype=np.uint8)  # 生成盒式卷积核
...
imgf3 = 255-imgf3  # 反相
plt.figure('连通域')
plt.subplot(211), plt.imshow(imgf1, 'gray'), plt.title(
    '图开运算后，面积筛选'), plt.axis('off')
plt.subplot(212), plt.imshow(imgf3, 'gray'), plt.title(
    '去除小黑点（反相），处理结束再反相'), plt.axis('off')
plt.show()
