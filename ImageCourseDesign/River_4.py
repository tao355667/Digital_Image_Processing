...
# %% 读图
img = cv.imread('./CourseDesignData/dataset-river/train/4_0_4461.png',
                cv.IMREAD_GRAYSCALE)  # 0表示灰度图
...
# canny边缘检测
imgBlur2 = cv.Canny(img, 0, 255)
# imgBlur2 = cv.addWeighted(img, 0.5, imgBlur2, 0.5, 0)
...
# %%
# 图像腐蚀
binaryMean = 255-binaryMean
kSize = (3, 3)  # 卷积核的尺寸
kernel = np.ones(kSize, dtype=np.uint8)  # 生成盒式卷积核
imgDilate3 = cv.dilate(binaryMean, kernel=kernel)  # 图像膨胀
kSize = (5, 5)  # 卷积核的尺寸
kernel = np.ones(kSize, dtype=np.uint8)  # 生成盒式卷积核
imgDilate5 = cv.dilate(binaryMean, kernel=kernel)
kSize = (7, 7)  # 卷积核的尺寸
kernel = np.ones(kSize, dtype=np.uint8)  # 生成盒式卷积核
imgDilate7 = cv.dilate(binaryMean, kernel=kernel)
kSize = (9, 9)  # 卷积核的尺寸
kernel = np.ones(kSize, dtype=np.uint8)  # 生成盒式卷积核
imgDilate9 = cv.dilate(binaryMean, kernel=kernel)

plt.figure('膨胀')
plt.subplot(221), plt.imshow(imgDilate3, 'gray'), plt.title('3x3')
plt.subplot(222), plt.imshow(imgDilate5, 'gray'), plt.title('5x5')
plt.subplot(223), plt.imshow(imgDilate7, 'gray'), plt.title('7x7')
plt.subplot(224), plt.imshow(imgDilate9, 'gray'), plt.title('9x9')
plt.show()
# %%
# 图像的开运算
# kSize = (3, 3)  # 卷积核的尺寸
